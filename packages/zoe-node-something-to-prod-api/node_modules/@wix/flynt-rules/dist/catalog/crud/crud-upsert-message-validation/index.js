...
exports.rule = {
    description: 'validates CRUD UPSERT method annotation',
    severity: flynt_engine_1.Severity.Error,
    run(ctx) {
        ctx.visit({
            visitMethod(method) {
                const crudOption = method.options.getTypedOption(wix_1.wix.api.crud);
                if (crudOption && crudOption.value.method === annotations_1.Crud.Method.UPSERT) {
                    (0, crud_utils_1.validateItemField)(crudOption.value.upsertOptions, method, method.request, ctx);
                    (0, crud_utils_1.validateFieldMask)(crudOption.value.upsertOptions, method, ctx);
                }
            }
        });
    }
