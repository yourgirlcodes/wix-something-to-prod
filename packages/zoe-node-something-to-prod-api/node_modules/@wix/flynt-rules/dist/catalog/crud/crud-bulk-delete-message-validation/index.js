...
exports.rule = {
    description: 'validates CRUD BULK_DELETE method annotation',
    severity: flynt_engine_1.Severity.Error,
    run(ctx) {
        ctx.visit({
            visitMethod(method) {
                const crudOption = method.options.getTypedOption(wix_1.wix.api.crud);
                if (crudOption && crudOption.value.method === annotations_1.Crud.Method.BULK_DELETE) {
                    (0, crud_utils_1.validateIdsField)(crudOption.value.bulkDeleteOptions, method, ctx);
                }
            }
        });
    }
