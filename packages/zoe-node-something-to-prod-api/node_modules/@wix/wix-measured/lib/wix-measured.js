const measured = require('measured-core'),
  ...
  SimpleMovingAverage = require('./simple-moving-average');
...
module.exports = class WixMeasured {
  ...
  meter(key, name) {
    ...
    const meter = this._registry.addMeter(resolvedKey, resolvedName, () => {
      const m = new measured.Meter({rateUnit: 60000});
      //TODO: clean up once https://github.com/felixge/node-measured/pull/30 is merged
      m['_m1Rate'] = new SimpleMovingAverage();
      return m;
    });
  ...
  gauge(key, name) {
    ...
    const logKey = `${resolvedKey}=${resolvedName}`;
    let value = () => 0;
    const createGauge = () => new measured.Gauge(() => {
    ...
    this._registry.addGauge(resolvedKey, resolvedName, createGauge);
  ...
  hist(key, name) {
    ...
    const logKey = `${resolvedKey}=${resolvedName}`;
    const hist = this._registry.addHist(resolvedKey, resolvedName, () => new Histogram());
...
class Histogram extends measured.Histogram {
  reset() {
